...
class LaserHistory_nocuts(Plugin):
        ...
        for i, EEsupermodule in enumerate(EEsupermodules_list):
            ...
            for c in self.channel_list:
              if c["SM"] == EBsupermodule:
                currentSM_ch_list.append(c)
            if len(currentSM_ch_list) == 0: continue
            self.folder = "EcalEndcap/EBLaserTask/Laser3/"
    ...
    def create_history_plots(self, save_path):
        ...
        #filling Laser > 300 history plot
        self.fill_history_subplots(df_high, available_runs, f"Laser_channels_greater_{UP_LIMIT}", f"{save_path}", change_hist_limits=True)
        #filling Laser < 100 history plot
        self.fill_history_subplots(df_low, available_runs, f"Laser_channels_less_{LOW_LIMIT}", f"{save_path}", change_hist_limits=True)

        run_df.loc[run_df.detector.str.contains("EB")].drop(columns=["label","detector","detector_priority"]).to_csv(f"{save_path}/EB_LaserHistory.csv", index=None)
